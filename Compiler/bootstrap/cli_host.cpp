...
int main(int argc,char**argv){
 if(argc<2){usage();return 0;}const std::string cmd=argv[1];
 if(cmd=="--version"||cmd=="version"){std::cout<<"noqeri "<<NOQERI_COMPILER_VERSION<<"\nlanguage="<<NOQERI_LANGUAGE_VERSION<<"\nedition="<<NOQERI_EDITION<<"\nabi="<<NOQERI_ABI_VERSION<<"\ndefault-target="<<NOQERI_DEFAULT_TARGET<<"\npackage-format="<<NOQERI_PACKAGE_FORMAT<<"\nregistry-protocol="<<NOQERI_REGISTRY_PROTOCOL<<"\n";return 0;}
 if(cmd=="targets"){for(const auto&target:TargetRegistry::supported()){auto info=TargetRegistry::resolve(target);std::cout<<target<<" pointer="<<(info.pointerWidth*8)<<" stack-align="<<info.stackAlignment<<(info.triple.isFreestanding()?" freestanding":" hosted-adapter")<<"\n";}return 0;}
 ...
 if(cmd=="db"){
  if(argc<3){std::cerr<<"NQR-D8041: db requires a .nqd script\n";return 1;}Diagnostics d;std::filesystem::path overridePath=argc>=4?std::filesystem::path(argv[3]):std::filesystem::path{};if(!NoqeriDatabase{}.execute(argv[2],overridePath,std::cout,d)){d.print(argv[2]);return 1;}return 0;
 }
 try{
  if(cmd=="assemble"){if(argc<4){usage();return 1;}const auto target=TargetTriple::parse(argc>=5?argv[4]:NOQERI_DEFAULT_TARGET);Diagnostics d;if(!AssemblerDriver{}.assemble(argv[2],argv[3],target,d)){d.print(argv[2]);return 1;}std::cout<<"assembled target object: "<<argv[3]<<" target="<<target.str()<<"\n";return 0;}
  if(cmd=="link"){if(argc<4){usage();return 1;}std::vector<std::filesystem::path>objects;for(int i=3;i<argc;++i)objects.emplace_back(argv[i]);Diagnostics d;if(!LinkerDriver{}.linkObjects(objects,argv[2],TargetTriple::parse(NOQERI_DEFAULT_TARGET),d)){d.print(argv[2]);return 1;}std::cout<<"linked artifact: "<<argv[2]<<"\n";return 0;}
  ...
  if(cmd=="lex"){auto source=readTextFile(path);Diagnostics d;Lexer lexer(source,d);auto tokens=lexer.lex();if(d.hasErrors()){d.print(path.string());return 1;}for(const auto&t:tokens)std::cout<<t.span.line<<':'<<t.span.column<<"  "<<tokenKindName(t.kind)<<"  "<<t.lexeme<<"\n";return 0;}
  ...
  if(cmd=="web"){std::filesystem::path output=argc>=4&&argv[3][0]!='-'?std::filesystem::path(argv[3]):std::filesystem::path("build")/(selected->projectName+".nqo");if(output.extension().empty())output+=".nqo";std::filesystem::create_directories(output.parent_path().empty()?std::filesystem::path("."):output.parent_path());Diagnostics d;if(!WebBackend{}.emitModule(compilation.ast,output,d)){d.print(path.string());return 1;}std::cout<<"emitted Noqeri web object: "<<output<<"\n";return 0;}
  if(cmd=="build"){const auto target=TargetRegistry::resolve(options.target);if(target.triple.architecture!=Architecture::X86_64){std::cerr<<"NQR-K5120: native backend currently implements x86_64 lowering; target contract "<<options.target<<" is recognized but has no backend yet\n";return 1;}std::filesystem::path output=argc>=4&&argv[3][0]!='-'?std::filesystem::path(argv[3]):std::filesystem::path("build")/(selected->projectName+".s");if(output.extension().empty())output+=".s";Diagnostics d;if(!NativeBackend{}.emitAssembly(compilation.nir,output,d)){d.print(path.string());return 1;}std::cout<<"emitted Noqeri x86-64 assembly: "<<output<<"\ntarget="<<options.target<<"\nentry=noqeri_entry(noqeri_abi*)\n";return 0;}
  usage();return 1;
