...
const __nqIndex = (value,index) => { __nqNonNull(value,'index base'); const length=Number(value.length ?? 0); if(!Number.isInteger(index) || index < 0 || index >= length) throw new RangeError(`Noqeri index out of bounds: index=${index}, length=${length}`); return value[index]; };
