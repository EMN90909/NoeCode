...
| NoqeriDB `.sql` | executable adapter over the same `.nqdb` store: CREATE TABLE, INSERT, SELECT projection/equality WHERE/simple ORDER BY, UPDATE, DELETE, transaction shadow/rollback; regression test | Implemented practical bootstrap subset; JOIN/GROUP BY/ALTER and full planner parity remain unsupported and fail closed |
