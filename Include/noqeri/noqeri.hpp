...
inline constexpr const char* NOQERI_LANGUAGE_VERSION = "1.3";
inline constexpr const char* NOQERI_COMPILER_VERSION = "1.3.0-systems";
...
enum class TokenKind {
    ...
    Import, Record, Class, Extern, Export, Volatile, As,
...
struct CallExpr final : Expr { ExprPtr callee; std::vector<ExprPtr> args; };
struct CastExpr final : Expr { ExprPtr value; std::string typeName; };
struct IndexExpr final : Expr { ExprPtr object,index; std::size_t elementSize=1; bool volatileAccess=false; };
...
struct FunctionStmt final : Stmt { std::string name; std::vector<Parameter> params; std::optional<std::string> returnType; std::shared_ptr<BlockStmt> body; bool isExtern=false; bool isExport=false; };
...
enum class TypeKind { Unknown,Void,Null,Bool,I8,I16,I32,I64,U8,U16,U32,U64,Isize,Usize,Int,Float,String,Pointer,Record };
struct Type {
...
enum class NirOp { Const,Load,Store,Unary,Binary,Cast,Call,AddressOf,LoadMemory,StoreMemory,PtrOffset,Jump,JumpIfFalse,Return,Nop };
struct NirInstruction { NirOp op=NirOp::Nop; std::optional<Reg> dest; std::string text; NirValue literal; std::vector<Reg> args; std::size_t target=0; std::size_t width=8; bool isVolatile=false; };
struct NirFunction { std::string name; std::vector<std::string> params; std::vector<NirInstruction> code; Reg nextReg=0; bool isExtern=false; bool isExport=false; };
