...
inline constexpr const char* NOQERI_COMPILER_VERSION="1.2.0-freestanding";
...
struct Token{TokenKind kind=TokenKind::Eof;std::string lexeme;Span span;}; const char* tokenKindName(TokenKind kind);
...
struct FunctionType{std::vector<Type>params;Type result;};
...
class Lowerer{public:NirProgram lower(const Program&program);private:void lowerStmt(NirFunction&fn,const StmtPtr&stmt);Reg lowerExpr(NirFunction&fn,const ExprPtr&expr);Reg emit(NirFunction&fn,NirInstruction instruction);};
...
class Formatter{public:std::string format(const std::string&source,Diagnostics&diagnostics)const;};class TestRunner{public:int runDirectory(const std::filesystem::path&dir)const;};class LanguageServer{public:int run();};
