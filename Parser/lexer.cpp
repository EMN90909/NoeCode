...
bool Lexer::match(char expected){if(peek()!=expected)return false;advance();return true;}
...
void Lexer::skipWhitespaceAndComments(){
    for(;;){
        ...
        if(c=='/'&&peek(1)=='*'){
            ...
            if(peek()=='\0'){diagnostics_.error("NOE-L1002",{current_,current_,line_,column_},"unterminated block comment");return;}
...
Token Lexer::identifier(){
    const auto start=current_-1,line=line_,col=column_-1;
...
Token Lexer::number(){
    ...
    if(source_[start]=='0'&&(peek()=='x'||peek()=='X')){
        ...
        return Token{TokenKind::Integer,source_.substr(start,current_-start),Span{start,current_,line,col}};
    }
    while(std::isdigit(static_cast<unsigned char>(peek())))advance();
    TokenKind kind=TokenKind::Integer;
    if(peek()=='.'&&std::isdigit(static_cast<unsigned char>(peek(1)))){
        kind=TokenKind::Float;advance();
        while(std::isdigit(static_cast<unsigned char>(peek())))advance();
    }
    return Token{kind,source_.substr(start,current_-start),Span{start,current_,line,col}};
}
Token Lexer::stringLiteral(){
    ...
    if(peek()=='\0'){diagnostics_.error("NOE-L1001",{start,current_,line,col},"unterminated string literal");return Token{TokenKind::String,source_.substr(start,current_-start),{start,current_,line,col}};}
    advance();
    return Token{TokenKind::String,source_.substr(start,current_-start),{start,current_,line,col}};
}
Token Lexer::scanToken(){
    skipWhitespaceAndComments();
    const auto start=current_,line=line_,col=column_;
    if(current_>=source_.size())return Token{TokenKind::Eof,"",{current_,current_,line_,column_}};
    char c=advance();
    ...
    switch(c){
        ...
        case '[':return make(TokenKind::LBracket,start,line,col); case ']':return make(TokenKind::RBracket,start,line,col);
        ...
        case '&':return make(match('&')?TokenKind::AndAnd:TokenKind::Ampersand,start,line,col);
        case '|':if(match('|'))return make(TokenKind::OrOr,start,line,col);break;
        case '"':return stringLiteral();
    }
    diagnostics_.error("NOE-L1000",{start,current_,line,col},std::string("unexpected character '")+c+"'");
    return make(TokenKind::Eof,start,line,col);
}
std::vector<Token> Lexer::lex(){
    std::vector<Token> out;
    for(;;){auto t=scanToken();out.push_back(t);if(t.kind==TokenKind::Eof)break;}
    return out;
}
} // namespace noe
