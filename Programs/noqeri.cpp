...
static bool usesRuntimeHost(const NirProgram& p){
    auto scan=[](const NirFunction& fn){for(const auto& i:fn.code)if(i.op==NirOp::Call&&(i.text=="host"||i.text=="clockMillis"||i.text=="platform"||i.text=="textLength"))return true;return false;};
    if(scan(p.entry))return true;
    for(const auto& fn:p.functions)if(scan(fn))return true;
    return false;
}
...
int main(int argc,char** argv){
    ...
    try{
        if(cmd=="new"){
            ...
            std::filesystem::path dir=argc>=4?argv[3]:argv[2];Diagnostics d;
        ...
        if(cmd=="manifest"||cmd=="lock"){
            std::filesystem::path path=argc>=3?argv[2]:"project.nqr";Diagnostics d;auto p=PackageManager{}.loadManifest(path,d);
            if(!p){d.print(path.string());return 1;}
            if(cmd=="manifest"){std::cout<<"name="<<p->name<<"\nversion="<<p->version<<"\nentry="<<p->entry<<"\nprofile="<<p->profile<<"\ntarget="<<p->target<<"\ndependencies="<<p->dependencies.size()<<"\n";return 0;}
        ...
        if(cmd=="format"){
            ...
            std::filesystem::path path=argv[2];auto source=readTextFile(path);Diagnostics d;auto text=Formatter{}.format(source,d);
            if(d.hasErrors()){d.print(path.string());return 1;}
            std::ofstream out(path,std::ios::trunc);out<<text;std::cout<<"formatted "<<path<<"\n";return 0;
        }
        Diagnostics selectionDiagnostics;auto selected=selectSource(argc,argv,selectionDiagnostics);
        if(!selected){selectionDiagnostics.print("project.nqr");return 1;}
        auto path=selected->source;auto source=readTextFile(path);
        if(cmd=="lex"){
            Diagnostics d;Lexer l(source,d);auto tokens=l.lex();if(d.hasErrors()){d.print(path.string());return 1;}
        ...
        if(cmd=="build"){
            if(usesRuntimeHost(c.nir)){
                std::cerr<<"NQR-N5010: host ABI services are runtime/embedding capabilities and are not linked into the freestanding bootstrap native target\n"
                         <<" help: use 'noqeri run', embed through abi.h, or keep native builds to language/user functions and print\n";
                return 1;
            }
            std::filesystem::path output;if(argc>=4)output=argv[3];else output=std::filesystem::path("build")/selected->projectName;
            std::filesystem::create_directories(output.parent_path().empty()?std::filesystem::path("."):output.parent_path());
            auto assembly=output;assembly+=".s";Diagnostics d;
            ...
            if(std::getenv("NOQERI_KEEP_ASM")==nullptr){std::error_code ec;std::filesystem::remove(assembly,ec);}
