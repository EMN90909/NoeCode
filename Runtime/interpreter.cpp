...
int Interpreter::run(const NirProgram&p){try{runFunction(p,p.entry,{});return 0;}catch(const std::exception&e){std::cerr<<"NQR-R4000: runtime error: "<<e.what()<<"\n";return 1;}}
    ...
    while(pc<fn.code.size()){
        ...
        switch(i.op){
            ...
            case NirOp::Call:{
                ...
                auto user=std::find_if(p.functions.begin(),p.functions.end(),[&](const auto&x){return x.name==i.text;});
                if(user!=p.functions.end()){
                    regs[*i.dest]=runFunction(p,*user,a);
                    break;
                }
                std::string error;
                if(i.text=="host"){
                    if(a.empty()||!std::holds_alternative<std::string>(a[0]))throw std::runtime_error("host requires a string service name");
                    std::string service=std::get<std::string>(a[0]);
                    std::vector<NirValue>hostArgs(a.begin()+1,a.end());
                    std::optional<NirValue> result;
                    if(host_) result=callAbiFunction(host_,service,hostArgs,error);
                    if(!result) result=callAbiFunction(defaultHostApi(),service,hostArgs,error);
                    if(!result) throw std::runtime_error(error.empty()?"unknown host service "+service:error);
                    regs[*i.dest]=*result;
                    break;
                }
                std::optional<NirValue> result;
                if(host_) result=callAbiFunction(host_,i.text,a,error);
                if(!result) result=callAbiFunction(defaultHostApi(),i.text,a,error);
                if(!result) throw std::runtime_error(error.empty()?"unknown function "+i.text:error);
                regs[*i.dest]=*result;
                break;
            }
