...
int runProductionDoctor(const std::filesystem::path& root) {
    ...
    const std::vector<Check> required = {
        ...
        {"Runtime/interpreter.cpp","reference interpreter"},{"Programs/noqeri.cpp","CLI entry point"},
    ...
    if (failures == 0) {
        std::cout << "noqeri production-track repository checks passed for the supported 1.0 subset.\n";
        return 0;
    }
