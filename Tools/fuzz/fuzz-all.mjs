#!/usr/bin/env node
import { mkdtempSync, mkdirSync, writeFileSync, rmSync, existsSync, copyFileSync, readFileSync, statSync } from 'node:fs'
...
const artifactRoot = resolve(opts.artifacts || 'build/fuzz-artifacts')
const replay = opts.replay ? resolve(opts.replay) : ''
...
function mutateText(seedText) {
  ...
  for (let e = 0; e < edits; e++) {
    ...
    else if (op === 3) s = s.slice(0, at) + pick(['function','record','import','let','const','while','repeat','return','unsafe','null','move','comptime','atomic.load','0','999999999999999999999']) + s.slice(at)
    else if (s.length) {
      const width = 1 + (rnd() % Math.min(24, s.length))
      const start = rnd() % Math.max(1, s.length - width + 1)
      const chunk = s.slice(start, start + width)
      s = s.slice(0, at) + chunk + s.slice(at)
    }
  }
  return s
}
...
const sourceCorpus = [
  ...
  'const base: int = 4\nfunction twice(x: int): int { return x * 2 }\nlet x: int = comptime(twice(base))\nprint(x)\n',
  'let owner: int = 7\n{ let view: *int = &owner print(*view) }\nlet next: int = move(owner)\nprint(next)\n',
  'record Job { id: u64, state: u64 }\nlet jobs: [Job; 2]\nlet i: usize = 0 as usize\nwhile i < len(jobs) { i = i + 1 as usize }\n'
]
...
function persistCrash(crash) {
  ...
  const id = `${Date.now()}-${crashes.length}`
  const saved = []
  for (const arg of crash.args) {
    try {
      if (!existsSync(arg) || !statSync(arg).isFile()) continue
      const target = join(artifactRoot, `${id}-${basename(arg)}`)
      copyFileSync(arg, target)
      saved.push(target)
    } catch {}
  }
  const metadata = join(artifactRoot, `${id}.json`)
  writeFileSync(metadata, JSON.stringify({ ...crash, seed, saved, replay: saved.find(x => x.endsWith('.nqr')) ? `node Tools/fuzz/fuzz-all.mjs --replay=${saved.find(x => x.endsWith('.nqr'))}` : null }, null, 2) + '\n')
  return metadata
}

function run(label, args, expected = new Set([0, 1])) {
  const r = spawnSync(noqeri, args, { encoding: 'utf8', timeout, maxBuffer: 4 * 1024 * 1024 })
  ...
  if (timedOut || signalCrash || badExit || r.error && !timedOut) {
    ...
    const signature = `${label}|${r.status}|${r.signal}|${stderr.split(/\r?\n/)[0] || r.error?.message || ''}`
    if (!signatures.has(signature)) {
      signatures.add(signature)
      const crash = { label, args, status: r.status, signal: r.signal, timedOut, error: r.error?.message || '', stderr }
      crashes.push(crash)
      crash.artifact = persistCrash(crash)
    }
    return false
  }
  return true
...
function replaySource(path) {
  if (!existsSync(path)) { console.error(`fuzz-all: replay file does not exist: ${path}`); return 2 }
  let ok = true
  ok = run('replay-lexer', ['lex', path]) && ok
  ok = run('replay-check', ['check', path]) && ok
  ok = run('replay-nir', ['nir', path]) && ok
  console.log(JSON.stringify({ result: ok ? 'ok' : 'crash', replay: path, seed, crashes }, null, 2))
  return ok ? 0 : 1
}

try {
  if (replay) {
    process.exitCode = replaySource(replay)
  } else {
    for (let i = 0; i < cases; i++) {
      const source = join(root, `case-${i}.nqr`)
      writeFileSync(source, mutateText(sourceCorpus[i % sourceCorpus.length]))
      run('lexer', ['lex', source])
      run('parser+type-checker+ownership+borrow+safety', ['check', source])
    ...
    if (crashes.length) {
      console.error(JSON.stringify({ result: 'crash', seed, completedCases: crashes.length ? undefined : cases, crashes, artifacts: artifactRoot }, null, 2))
      process.exitCode = 1
    } else {
      console.log(JSON.stringify({ result: 'ok', seed, cases, elapsed_ms: elapsedMs, cases_per_second: Number((cases * 1000 / elapsedMs).toFixed(2)), targets: ['lexer','parser/type-checker/ownership/borrow/safety','lowerer','database/.nqd','package-manifest','sql/url/dns/http','object-loader/linker'] }))
    }
  }
} finally {
