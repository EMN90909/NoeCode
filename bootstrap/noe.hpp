...
struct UnaryExpr final : Expr { TokenKind op; ExprPtr operand; };
...
enum class TypeKind { Unknown, Void, Null, Bool, Int, Float, String };
struct Type {
...
enum class NirOp { Const, Load, Store, Unary, Binary, Call, Jump, JumpIfFalse, Return, Nop };
...
class Optimizer {
public:
    void optimize(NirProgram& program) const;
private:
    void optimizeFunction(NirFunction& fn) const;
};
...
class NativeBackend {
public:
    bool emitAssembly(const NirProgram& program, const std::filesystem::path& output, Diagnostics& diagnostics) const;
};
class LinkerDriver {
public:
...
class Formatter {
public:
    std::string format(const std::string& source, Diagnostics& diagnostics) const;
};
class TestRunner {
public:
    int runDirectory(const std::filesystem::path& dir) const;
};
