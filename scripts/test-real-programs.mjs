...
await scenario("persistent NoqeriDB CRUD/readback/duplicate rejection", async () => {
...
await scenario("web backend emits and executes static ESM", async () => {
...
await scenario("database library service", async () => {
  expectOutput(join(integration, "database_service.nqr"), "database-service: ok");
});

await scenario("JSON/token service", async () => {
  expectOutput(join(integration, "json_service.nqr"), "json-service: ok");
});

await scenario("scheduler/concurrency policy", async () => {
  expectOutput(join(integration, "concurrency_service.nqr"), "concurrency-service: ok");
});

await scenario("host-facing HTTP APIs compile and lower", async () => {
  const source = join(integration, "backend_host_api.nqr");
  invoke(["check", source]);
  invoke(["nir", source]);
});

await scenario("host-facing storage APIs compile and lower", async () => {
  const source = join(integration, "storage_host_api.nqr");
  invoke(["check", source]);
  invoke(["nir", source]);
});

if (failures.length) {
  console.error(`real-program integration suite: ${failures.length} scenario(s) failed`);
